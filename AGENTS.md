...
### i18n (Bilingual)
- Translation dictionary at `src/i18n/ui.ts` — keys: `nav`, `hero`, `about`, `services`, `featured`, `process`, `testimonials`, `faq`, `footer`, `projects`, `seo`.
...
### SEO
- Per-page meta descriptions from `seo.homeDesc` / `seo.projectsDesc` in i18n dictionary.
- Project detail pages use the project's own `image` as OG/Twitter image.
- `og:locale` + `og:locale:alternate`, `twitter:site`, `og:image:width/height`.
- `hreflang` alternate links for `id`, `en`, `x-default`.
- JSON-LD: `WebSite` (all pages), `Person` (home), `BreadcrumbList` (project detail).
- Sitemap auto-generated by `@astrojs/sitemap`.

### Project schema (`src/content.config.ts`)
- `category` is `z.array(z.string()).optional()` — supports multiple categories per project.
- Projects grid filter uses space-separated `data-category` + `split/includes` matching.
- A special GitHub card sits at the end of the grid (always visible, `data-github` attribute skips filter).

### Deployment
- Pure static output to `dist/`. No SSR, no env vars needed.
- Node `>=22.12.0` required only for build.
- Works on Vercel, Netlify, Cloudflare Pages, GitHub Pages, VPS + Nginx/Caddy.
